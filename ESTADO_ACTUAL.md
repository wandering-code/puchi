...
- [ ] **Abrir puertos en el router de casa hacia el mini PC (192.168.1.106) para que el TURN propio funcione de verdad** — TCP+UDP 3478 y UDP 49160-49260. Sin esto, coturn ya corre en producción pero es inalcanzable desde fuera de la LAN — las llamadas entre redes distintas seguirán fallando igual que antes. Pendiente del usuario (ver sección "TURN propio (coturn)" en Despliegue). Una vez abierto, repetir la prueba real entre iPhones en redes distintas que motivó todo esto (2026-07-21).
- [ ] **Seguir probando la llamada grupal de #club-general con dispositivos reales** — implementada 2026-07-17 (mesh P2P); con el TURN propio ya en marcha (pendiente de abrir puertos, ver arriba) falta validar de nuevo con 2+ personas de verdad: calidad con varios vídeos a la vez, reconexión si alguien pierde red, comportamiento en móvil (grid + PiP + lanzador flotante conviviendo).
...
- **Pendiente del usuario** (bloqueante para que esto sirva de algo): abrir en el router de casa, hacia `192.168.1.106`, **TCP+UDP 3478** y **UDP 49160-49260**. Sin esto coturn corre pero es inalcanzable desde fuera de la LAN de casa (se puede comprobar que responde localmente, pero no desde otra red) — ver el punto en "Pendiente" al principio del documento.
