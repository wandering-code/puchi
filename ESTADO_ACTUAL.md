...
> **2026-07-21**: el usuario reportó que, probando llamadas 1-to-1 y grupales entre iPhones reales, cada uno solo veía su propia cámara — nunca la del interlocutor. Dos causas encontradas: (1) a la llamada 1-to-1 le faltaba el mismo fix de autoplay (`.play()` con reintento) que ya tenía la grupal desde el 2026-07-17 — el vídeo remoto se quedaba en negro por el bloqueo de autoplay del navegador; (2) la señalización solo tenía servidores **STUN** (Google) — insuficiente cuando algún dispositivo está detrás de NAT simétrico (frecuente en redes móviles), que es probablemente lo que pasó de verdad en la prueba. Se evaluaron proveedores TURN de terceros (Metered, Twilio, Cloudflare Calls) pero, dado el patrón de uso real del club (pocas llamadas al mes, pero largas — 3h+, hasta 3 personas en malla), salían caros o con cuota insuficiente (ver números en la sección de despliegue); se optó por **coturn propio** en el mini PC — gratis, sin límite de GB, de sobra con los 500 Mbps de subida de casa. Ver sección "TURN propio (coturn)" bajo Despliegue para toda la config.
>
> El usuario abrió los puertos del router (Livebox/Orange) el mismo día — verificado desde una red externa que coturn responde por TCP y UDP en el 3478. Pruebas reales posteriores (iPhone con red externa + Mac) sacaron dos bugs más, ya arreglados: (3) **condición de carrera real** en la señalización — el array de candidatos ICE pendientes se vaciaba con `for..of` + `= []` al final en vez de `splice(0)` atómico, perdiendo candidatos que llegaran mientras el bucle aún estaba en un `await` anterior (por eso a veces conectaba y a veces no); de paso, los mensajes de señalización del WS ahora se procesan en cola estrictamente en serie, y `'disconnected'` ya no cuelga la llamada al instante (margen de 8s por si se recupera solo — solo `'failed'`/`'closed'` cuelgan ya mismo); (4) el **vídeo local** (el propio PiP) también se quedaba en negro en iOS Safari pese a ir silenciado — confirmado en real (quitar/poner la cámara lo "despertaba") — mismo fix de `.play()` con reintento que ya tenía el remoto, aplicado también al local en `CallView` y `GroupCallView`. Con estos cuatro fixes la videollamada real entre iPhone (red externa) y Mac (wifi) quedó estable. Dos features nuevas pedidas de paso: **tono de llamada entrante** (sintetizado con Web Audio, sin fichero de audio — dos notas suaves D5/A5 con fade in/out, en bucle mientras suene, `AudioContext` preparado en el primer toque del jugador para no chocar con las restricciones de autoplay) y **colgar la llamada anterior al aceptar una nueva** (1-to-1 o grupal) — antes una segunda llamada entrante pisaba el estado de la primera sin colgarla de verdad ni localmente ni para el interlocutor; ahora aceptar la nueva cuelga la anterior limpiamente, y rechazarla la restaura tal cual sin tocarla. Después, **aviso de llamada perdida** — vibración (solo Android, iOS Safari no tiene `navigator.vibrate`) + notificación nativa del sistema si no contestas a tiempo o el que llama cuelga antes, aunque la app esté minimizada; a propósito solo llamadas perdidas, no cuando cuelgan una ya activa. Reportado también un desalineado de avatares con icono/emoji frente a foto en el sidebar de Diskordkito — `PlayerAvatar.jsx` solo fijaba el `font-size` del emoji, pero la caja real de un emoji no coincide con el em-square exacto de la fuente; arreglado dándole al emoji la misma caja `width/height:size` exacta que al `<img>`, centrada con flex.
>
> **Sustituidos los emojis de avatar por una galería de imágenes + foto propia ya en el registro** (mismo 2026-07-21) — se quitó el selector de emojis (registro y Ajustes), sustituido por una galería de 10 avatares ilustrados originales (`frontend/public/avatars/*.svg` — animales graciosos y un par de iconos estilo meme dibujados a mano, sin fotos de terceros con copyright, para evitar el problema de derechos de autor de usar memes reales) más la opción de subir tu propia foto — esto último ya disponible desde el propio formulario de registro, no solo en Ajustes como antes (reutiliza `AvatarCropModal` tal cual). Como el registro no da sesión hasta que un admin lo aprueba, no había forma de llamar después a `POST /players/me/avatar` (exige sesión) — se cambió `/auth/register` de JSON a multipart para poder mandar la foto (o la URL del preset elegido) en el mismo request; `_save_avatar_file` extraída como helper compartido entre registro y subida normal. `PATCH /players/me/profile` acepta ahora `avatar_url` además de `avatar_emoji`. Las cuentas existentes con solo emoji (sin foto) se siguen viendo igual — no se tocó el modelo de datos ni el render de fallback de `PlayerAvatar`, solo la UI de selección. Probado end-to-end con Playwright (registro con preset, registro con foto propia recortada, cambio de avatar en Ajustes) antes de desplegar — sin errores de consola, verificado en BBDD que queda el `avatar_url` correcto en los tres casos.
>
> **Reordenados los sitios de entrada a Ajustes/Admin** (mismo 2026-07-21) — antes ambos aparecían duplicados en el menú GatOS, el Dock y el botón flotante de móvil a la vez. Ahora cada uno tiene un único sitio: **Ajustes solo desde el menú GatOS** (quitado del Dock y del abanico del lanzador móvil, `launcherApps()` en `MobileLauncher.jsx` excluye `'settings'` a propósito); **Admin solo desde el Dock (escritorio) o el botón flotante (móvil)**, quitado del menú GatOS (`MenuBar.jsx`).
>
> **Ajustes: color de cuenta editable + todo el bloque de Identidad pasa a guardarse en borrador** (mismo 2026-07-21) — antes el color solo se elegía al crear la cuenta; añadido el mismo selector en Ajustes (paleta compartida `colorOptions.js`, ya no duplicada con `LoginScreen.jsx`). De paso, cambiar el avatar (galería o foto) dejó de aplicarse al instante como hacía antes — ahora, igual que el nombre, se queda en un borrador local (`pendingAvatar` en `SettingsApp.jsx`) hasta pulsar "Guardar cambios", que aplica nombre + color + avatar juntos en un solo paso; el botón se deshabilita si no hay nada pendiente. Probado con Playwright: cambios en pantalla sin guardar no tocan la BBDD; al guardar, se aplican los tres a la vez.
>
...
**⚙️ Ajustes** — reducida (2026-07-17) a cuenta: identidad (nombre, icono) + seguridad (PIN, borrado). Panel único con tres tarjetas: "Identidad", "Cambiar PIN" y "Zona de peligro" — ya no tiene pestañas. El fondo de escritorio se quitó de aquí y vive en Pirestore (ver arriba). Al subir una foto de perfil se abre antes un modal para encuadrarla (`AvatarCropModal.jsx`: arrastrar para mover, deslizador para zoom, recorte hecho en canvas en el propio navegador). La tarjeta "Identidad" permite: 
  ...
  - Cambiar el **avatar**: desde el 2026-07-21 ya no hay selector de emojis — o eliges uno de la **galería de 10 presets ilustrados** (`PRESET_AVATARS`, `frontend/src/utils/presetAvatars.js`, ficheros en `frontend/public/avatars/*.svg`) o subes una **foto** propia (`POST /players/me/avatar`, multipart, guarda en `backend/uploads/avatars/`, mismo patrón que las portadas de libro). Ambos casos escriben `Player.avatar_url` (preset = ruta estática, foto = ruta subida) y son inmediatos, sin botón "Guardar cambios" (ese botón solo aplica al nombre). La misma galería está disponible también **al crear la cuenta** (`LoginScreen.jsx`, antes solo se podía elegir emoji ahí) — `POST /auth/register` es multipart (no JSON) para poder mandar la foto recortada o la URL del preset en el mismo request, ya que una cuenta recién registrada queda `pending` sin sesión y no hay forma de llamar después a `POST /players/me/avatar`.
  - La foto/avatar sustituye al emoji **en toda la app** (no solo en Ajustes): mensajes de chat, llamadas 1-to-1 y grupales, sidebar de Diskordkito, MenuBar, LoginScreen, notificaciones. Se centralizó en un componente nuevo, `PlayerAvatar.jsx` (`src/components/gatos/`), que decide entre `<img>` y emoji según si `avatar_url` está presente. Backend: `Player.avatar_url` (nullable), incluido en `_player_out` y en `_message_out` (como `player_avatar_url`). El campo `avatar_emoji` sigue en el modelo (columnas no se han borrado) solo por compatibilidad con cuentas antiguas que aún no tienen `avatar_url` — no se puede volver a elegir emoji desde ningún sitio nuevo.
...
- [x] ~~Abrir puertos en el router de casa hacia el mini PC~~ — hecho el 2026-07-21 (Livebox/Orange), verificado desde red externa que coturn responde. Videollamada real iPhone↔Mac ya estable tras esto + los fixes de condición de carrera/autoplay local del mismo día.
- [ ] **Seguir probando la llamada grupal de #club-general con 3 dispositivos reales a la vez** (el club completo) — validado ya 1-to-1 entre dos redes distintas (2026-07-21); falta la prueba con los 3 miembros del club a la vez en la grupal: calidad con varios vídeos simultáneos, reconexión si alguien pierde red, comportamiento en móvil (grid + PiP + lanzador flotante conviviendo).
...
- **TURN propio (coturn) en vez de un proveedor de terceros** (2026-07-21) — con el patrón de uso real del club (pocas llamadas al mes pero de 3h+, hasta 3 personas) los proveedores con cuota gratuita se agotan en minutos y los planes de pago decentes empiezan en $99/mes; un proveedor con pago por uso (Cloudflare Calls, Twilio) habría costado céntimos por llamada, pero el usuario prefirió no depender de ningún pago externo pudiendo permitírselo con el ancho de banda de casa (500 Mbps subida, de sobra). Ver sección de Despliegue para la config exacta.
...
### TURN propio (coturn) para llamadas WebRTC (2026-07-21)

Motivo: solo STUN no basta para conectar dos dispositivos si alguno está detrás de NAT simétrico — es lo que probablemente pasó en la prueba real entre iPhones del 2026-07-21 (cada uno solo veía su propia cámara). Ver la entrada de esa fecha al principio del documento para la comparativa de proveedores considerada antes de decidir coturn propio.

- **Servicio `coturn`** en `docker-compose.prod.yml` (imagen `coturn/coturn:4.6-alpine`), `network_mode: host` — necesario porque el rango de puertos de relay UDP tiene que ser accesible directamente (mapear cientos de puertos uno a uno vía Docker sería frágil). Es el **único servicio del mini PC con puertos expuestos directamente a internet**, fuera del Cloudflare Tunnel — el Tunnel no sirve para esto porque el TURN necesita que cualquier cliente arbitrario (un iPhone en cualquier red) se conecte directo por UDP, no vía un cliente Zero Trust.
- **Credenciales efímeras vía HMAC** (mecanismo REST estándar de coturn, flag `--use-auth-secret`): el backend (`GET /turn-credentials` en `main.py`, autenticado) genera usuario `"<timestamp_expiry>:<player_id>"` + contraseña `base64(HMAC-SHA1(secret, usuario))`, válida 24h. Nunca se expone ningún secreto al frontend — solo las credenciales ya firmadas y de corta duración. El frontend (`GatOS.jsx`) las pide de nuevo (`refreshIceServers()`) al empezar cada llamada 1-to-1 o al unirse a la grupal, cacheándolas en un ref para toda la sesión de esa llamada; si el fetch falla o no hay TURN configurado, cae a un STUN-only hardcodeado (`FALLBACK_ICE_SERVERS`) — solo funcionaría entre dispositivos en la misma red, pero no rompe nada.
- **Secretos en dos archivos separados, ninguno en el repo** (ambos gitignored, con su `.example` documentado): `backend/.env` tiene `COTURN_STATIC_SECRET` + `COTURN_EXTERNAL_IP` (lo que necesita el backend para firmar credenciales); `coturn.env` (raíz del repo) tiene esos dos más `COTURN_INTERNAL_IP`, solo para el contenedor `coturn` — así coturn no hereda `SECRET_KEY`/`DATABASE_URL` del backend sin necesitarlos.
- **IP pública fija** de la casa: `92.187.124.81`. IP LAN del mini PC: `192.168.1.106`. Con `network_mode: host`, coturn ve todas las interfaces del host (incluidos los bridges de Docker de otros proyectos) — hay que restringirlo explícitamente con `--listening-ip`/`--relay-ip=192.168.1.106` (si no, `--external-ip` con solo el par público/privado no basta: seguía intentando relayar también por las interfaces internas de Docker, con el warning "NO EXPLICIT RELAY ADDRESS(ES) ARE CONFIGURED"). Formato de `--external-ip` cuando pública y privada difieren: `PUBLIC/PRIVATE` (no al revés).
- **Rango de puertos acotado a propósito**: en vez del rango completo típico (49152-65535), solo **49160-49260** (~100 puertos) — de sobra para el uso real (máximo 3 personas en malla, como mucho 3 conexiones relayed a la vez) y reduce la superficie expuesta en el router.
- **Validado antes de tocar producción**: se probó primero en un contenedor `docker run` suelto en el propio mini PC (sin tocar el compose real) con `turnutils_uclient -y -m 2 -W <secret>` — relay de cliente a cliente con 0% de paquetes perdidos, confirmando que la config y el esquema de credenciales HMAC son correctos antes de desplegarlo de verdad.
- **Puertos abiertos en el router** (hecho 2026-07-21, Livebox/Orange): `192.168.1.106` reservado por DHCP + reglas NAT/PAT `TCP+UDP 3478` y `UDP 49160-49260` → `192.168.1.106`. Verificado desde una red externa (TCP connect + respuesta STUN real por UDP).
- Sin TLS (`--no-tls --no-dtls`) — el contenido audio/vídeo ya va cifrado por SRTP (WebRTC) independientemente de si el transporte TURN es TLS o no; TURNS quedaría como posible endurecimiento futuro, no urgente para un uso de club pequeño.
